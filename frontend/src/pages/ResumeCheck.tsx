...
export default function ResumeCheckPage() {
  ...
  const checkMutation = useMutation({
    ...
    onError: (error: Error) => {
      // If the stored resume_id is stale (404), clear it and let the user re-upload
  ...
  const canCheck = jdText.trim() && (hasStoredResume || useVariant || file);
